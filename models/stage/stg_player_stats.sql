...
select  {{ dbt_utils.surrogate_key(['player', 'date']) }} as key
        , {{ dbt_utils.surrogate_key(['player']) }} as player_key
        , {{ dbt_utils.surrogate_key(['team']) }} as team_key
        , {{ dbt_utils.surrogate_key(['opponent']) }} as opponent_team_key
        , player, date
